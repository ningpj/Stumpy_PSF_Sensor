...
This case has been based around the excellent `PSF v1.0 / 1.1` Proportional Sync-Feedback sensor kit from [**Kashine6**](https://github.com/kashine6/Proportional-Sync-Feedback-Sensor?tab=readme-ov-file) (Discord: @jacksky6) and is intended to be used with the **Happy-Hare** MMU ecosystem and new Flowguard / Sync-feedback controller facilities (Happy-Hare `3.4.2` or later - currently in open beta). It has a total of `14.5 mm` of buffer range and will work with `20 mm` or `25 mm` springs from the kit (`20 mm` prefered).
