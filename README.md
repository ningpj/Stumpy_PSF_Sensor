# "_Stumpy_" Proportional Sync-Feedback (PSF) Sensor Enclosure for Multi Material Printing
...
This enclosure has been based around the excellent `PSF v1.0 / 1.1` Proportional Sync-Feedback sensor kit from [**Kashine6**](https://github.com/kashine6/Proportional-Sync-Feedback-Sensor?tab=readme-ov-file) (Discord: @jacksky6) and is intended to be used with the **Happy-Hare** MMU ecosystem and new Flowguard / Sync-feedback controller facilities (Happy-Hare `3.4.2` or later). `14.5 mm` of buffer range and works with `20 mm` or `25 mm` compression springs from the kit (`20 mm` preferred).
...
- Straighten the end of the PTFE tube you plan to use between the MMU and “Stumpy” PSF sensor. Avoid using PTFE with an `ID = 3.0 mm`. It klinks too easily especially if it needs to bend to accomodate the full range of gates / lanes which will jam the shuttle rather than moving freely for all selected gates.
